...
namespace Information_Management_System
{
    public partial class Equipments : Form
    {
        ...
        void GetEquip()
        {
            ...
            conn.Close();
            DGVequip.EnableHeadersVisualStyles = false;
            DGVequip.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;  // Make columns auto resize

            // Check if the column exists and then set the currency format
            if (DGVequip.Columns.Contains("Purchase Cost"))
            {
                DGVequip.Columns["Purchase Cost"].DefaultCellStyle.Format = "C2";  // Format as currency with 2 decimal places
            }
