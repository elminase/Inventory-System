...
{
    public partial class Materials_Admin_ : Form
    {
        ...
        void GetMats()
        {
            ...
            DGVmatAdmin.DataSource = dt;
            // Close the database connection
            conn.Close();
            DGVmatAdmin.EnableHeadersVisualStyles = false;
            DGVmatAdmin.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;  // Make columns auto resize

            // Check if the column exists and then set the currency format
            if (DGVmatAdmin.Columns.Contains("Purchase Cost"))
            {
                DGVmatAdmin.Columns["Purchase Cost"].DefaultCellStyle.Format = "C2";  // Format as currency with 2 decimal places
            }

        }

        private void Materials_Admin__Load(object sender, EventArgs e)
        {
            GetMats();
        ...
        private void picboxDeduct_Click(object sender, EventArgs e)
        {
            Deduct_Materials_ Deduct_Materials = new Deduct_Materials_();  // Deduct Materials Form
            Deduct_Materials.Show();
        }
        ...
        private void RefreshDataGrid(object sender, EventArgs e)
        {
            ...
            GetMats(); // Re-load the data into the DataGridView
        }
    }
}
